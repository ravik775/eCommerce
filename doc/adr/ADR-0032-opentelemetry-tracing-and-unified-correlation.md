...
| Drop `OrderCorrelationScope`'s order-ID substitution; propagate the **original** correlation ID through every Kafka event payload instead | One ID, genuinely front-to-back, matches ADR-0023's original intent exactly | Every saga event record (`OrderCreatedEvent`, `InventoryReservedEvent`, `PaymentSuccessEvent`, etc.) needs a new `correlationId` field, and every producer/consumer needs updating — real but mechanical work, same shape as the `OrderCorrelationScope` rollout already done |

**Chosen**: propagate the original correlation ID through event payloads. Order ID remains useful and stays as a *separate*, additional, always-logged field (still valuable for "find everything about order #23" queries) — but it no longer *replaces* the correlation ID.
