...
**Verification**: `./mvnw -pl api-gateway -am test` — `BUILD SUCCESS`, all existing tests (including `fallbackToRandomUuidIsAuditLogged`) pass unchanged. Live re-verification (real orders, checking `TRACE_ID_FALLBACK_UUID` frequency post-redeploy) still the actual proof this correction works — pending the redeploy that follows this commit.
