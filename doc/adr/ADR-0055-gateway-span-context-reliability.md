...
- Positive: even if the fallback were to still fire occasionally under some untested condition, it remains a visible, auditable, countable event instead of a silent, undetectable one (the 2026-08-17 17:30 IST correction's live test found zero occurrences across 10 real requests, but this safety net stays regardless).
...
### 2026-08-17 17:30 IST correction — Mono.defer alone wasn't enough; explicit ContextSnapshot restoration

Live re-verification (per this ADR's own pending item) found `TRACE_ID_FALLBACK_UUID` still firing after the `Mono.defer` fix — not eliminated, only reduced. Root-caused: `Mono.defer` moves the `Span.current()` read to subscription time, but it still depends on Reactor's *automatic* context propagation (`Hooks.enableAutomaticContextPropagation()`) having already re-threaded the correct OTel `Context` onto whatever thread the deferred lambda happens to execute on, **by the time it runs**. That's a timing assumption, not a guarantee — and evidently one that doesn't always hold under this gateway's real scheduling pattern.

**Fix**: replaced `Mono.defer(...)` with `Mono.deferContextual(contextView -> ...)`, and inside it, explicitly restored ThreadLocals from Reactor's own `ContextView` using `io.micrometer.context.ContextSnapshotFactory` (already on the classpath transitively via `micrometer-tracing-bridge-otel`; confirmed the exact API — `ContextSnapshotFactory.builder().build().captureFrom(contextView).setThreadLocals()`, returning an `AutoCloseable` `Scope` — by decompiling `context-propagation-1.1.4.jar` directly with `javap`, since the method name differs across versions and guessing wrong would have been a silent compile-time-only check, not a runtime guarantee). Reactor's `ContextView` is guaranteed correct throughout the whole reactive chain regardless of thread hops — that's Reactor's own core contract, independent of the ThreadLocal-bridge timing race this ADR's first fix still implicitly trusted. `Span.current()`/`Context.current()` inside the `try`-with-resources scope now read a value explicitly and verifiably restored moments earlier, not one hoped to already be there.

**Verification**: `./mvnw -pl api-gateway -am test` — `BUILD SUCCESS`, all existing tests (including `fallbackToRandomUuidIsAuditLogged`) pass unchanged. **Live re-verification, post-redeploy: 10 real checkout requests, 10/10 returned a valid 32-hex OTel-format `X-Trace-Id` (zero UUID fallbacks), and zero `TRACE_ID_FALLBACK_UUID` audit events fired in Loki across the whole test window.** This is genuinely closed, not just theoretically fixed — status upgraded accordingly.
