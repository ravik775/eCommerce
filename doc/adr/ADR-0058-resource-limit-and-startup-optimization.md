...
3. ~~**Enabled `spring.main.lazy-initialization`**~~ — **reverted, see the 2026-08-17 16:20 IST correction section below.** Broke the entire choreography saga silently (`@KafkaListener` beans never start consuming under lazy init).
...
### 2026-08-17 16:20 IST correction — lazy initialization silently broke the entire saga

Live regression-script run after the quota fix (above) found order 79 stuck in `CREATED` forever — not slow, genuinely stuck. Investigation: `inventory-service` showed **zero Kafka-related log lines at all** since its restart — no consumer-group join, no partition assignment, nothing. `spring.main.lazy-initialization=true` (this ADR's item 3) is a documented Spring Boot gotcha for exactly this case: `@KafkaListener`-annotated beans (`InventorySagaConsumer`, `PaymentSagaConsumer`, `OrderSagaConsumer`, `NotificationEventConsumer`) are never directly injected or called by application code — they're pure event listeners — so under lazy init, nothing ever triggers their creation, and their listener containers never start. This wasn't checked against before shipping the change.

**Fix**: reverted `SPRING_MAIN_LAZY_INITIALIZATION` entirely (`configmap-common.yaml`) rather than attempting a per-bean `@Lazy(false)` carve-out under time pressure — not worth the risk of missing another bean with the same issue. Items 1 (CPU limit raise) and 2 (liveness-probe widening) are the changes that are actually proven safe and effective (2-minute redeploys, zero restarts, confirmed across two separate rollouts) — item 3 is withdrawn.

**Process lesson, stacking on the quota one above**: two of this ADR's four items caused real, live regressions when applied (quota exhaustion, then a silently-broken saga) — both were config/infrastructure changes that looked safe in isolation but had a real interaction with existing behavior that wasn't verified before shipping. The CPU-limit-and-probe-timing pair (items 1-2) needed no correction and are the actual validated fix; lazy initialization needed a live functional test (place a real order, confirm it completes), not just a successful pod-Ready check, and didn't get one before being declared part of the fix.
