...
| [ADR-0063](../adr/ADR-0063-gateway-force-trace-context-reliability.md) | Partial: fixed a real Span.current()-reliability bug in the gateway's X-Force-Trace attribute stamping (captured Span reference directly, sidestepping async context timing). But live verification found the deeper cause of "gateway spans missing for authenticated routes": Spring Cloud Gateway's HTTP-server span for any *proxied* route (through NettyRoutingFilter) never reports to Tempo at all, regardless of sampling/attributes - only gateway-local routes (/actuator/prometheus, /user/me) ever get a span. Still open; needs the OTel Java agent or manual span wrapping, not undertaken here |
