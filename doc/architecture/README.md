...
| [ADR-0055](../adr/ADR-0055-gateway-span-context-reliability.md) | Gateway's X-Trace-Id generation: Mono.defer alone still left the fallback firing sometimes (found live) - corrected to Mono.deferContextual + explicit ContextSnapshotFactory restoration from Reactor's own ContextView, not trusting automatic ThreadLocal-bridge timing. Fallback path audit-logged either way |
