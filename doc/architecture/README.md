...
| [ADR-0055](../adr/ADR-0055-gateway-span-context-reliability.md) | Gateway's X-Trace-Id generation: Mono.defer alone still left the fallback firing sometimes - corrected to Mono.deferContextual + explicit ContextSnapshotFactory restoration from Reactor's own ContextView. Live-verified genuinely fixed: 10/10 real requests returned valid OTel-format IDs, zero fallback events. Regression check upgraded WARN->FAIL accordingly |
...
| [ADR-0065](../adr/ADR-0065-loki-audit-field-visibility-gap.md) | Payment-service's PAYMENT_SUCCESS log showed orderId/auditEvent in Grafana's Fields panel but not transactionId/amount, despite AuditLogger logging all four the same way - Loki's Alloy pipeline only ever regex-extracted 5 field names, never updated as more AuditLogger.with(...) call sites were added across services. Fixed generically via stage.logfmt + stage.structured_metadata (not stage.labels, to avoid blowing up index cardinality with high-cardinality values like transactionId) |
| [ADR-0064](../adr/ADR-0064-otel-java-agent-for-gateway-proxied-spans.md) | Closes ADR-0056/ADR-0063's remaining api-gateway gap for real: adopts the OpenTelemetry Java agent for api-gateway only (bytecode-instruments reactor-netty/Spring Cloud Gateway directly, anchored to the actual proxied I/O call rather than the framework's own unreliable self-reporting). New otel-agent-extension module reimplements the force-trace/error-always-sample export policy in the agent's isolated extension classloader. Other 4 services stay on library instrumentation (already confirmed working) |
| [ADR-0063](../adr/ADR-0063-gateway-force-trace-context-reliability.md) | Fixed a real Span.current()-reliability bug in the gateway's X-Force-Trace attribute stamping. Live verification then found the deeper cause of "gateway spans missing for authenticated routes": Spring Cloud Gateway's HTTP-server span for any *proxied* route never reports to Tempo at all under library instrumentation. Closed for real by ADR-0064 (OTel Java agent for api-gateway) - live-verified 3/3 |
| [ADR-0062](../adr/ADR-0062-cross-saga-span-links.md) | Saga-hop spans (order/inventory/payment/notification-service) are real but structurally disconnected in Tempo (outbox pattern publishes async, after the originating span ends). Propagates traceId+spanId of order-service's root request through every hop's outbox row/Kafka/RabbitMQ header, and OrderCorrelationScope now calls Span.addLink() (not setParent - a span can only have one parent, and this saga fans out to multiple consumers per event) so every hop's span links back to its true origin |
| [ADR-0061](../adr/ADR-0061-ui-surfaces-full-saga-trace-id.md) | UI's checkout success message showed a truncated correlationId prefix that failed exact-match Tempo/Loki search - reported live as "not useful." Now shows the full appTraceId (already on the X-Trace-Id response header), labeled "Saga ID", with a copy button and a pointer to the Order Trace Explorer dashboard |
